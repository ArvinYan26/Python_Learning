...
class NetworkDataClassification():
    ...
    def generate_labeled_data(self, per_class_data_len, num_classes):
        ...
        print(data_x)
        data_y = np.zeros(self.data_len) #矩阵长度就是维度
        #print(data_y) 1*15的全零矩阵
        data_y = np.split(data_y, num_classes) #
        print(data_y)
    ...
    def plot_data(self, data, num_classes):
        ...
        per_class_data_len = int(data.shape[0] / num_classes) #shape[0]:表示矩阵维度，shape[1]:表示矩阵列数
        ...
        return 1  #如果结束时返回1那么代表函数为正常结束，有异常，如果返回0表示正常结束
    ...
    def init_network(self):
        ...
        for i in range(self.num_classes):  # 循环每一类数据
            ...
            current_data = self.train_x[self.per_class_data_len * i:self.per_class_data_len * (i + 1), :]  #切片取值，
    ...
    def add_node_to_net(self, class_num, node_value, node_name='new_node'):
        ...
        self.G.add_node(node_name, class_num=-1, value=node_value)  #-1:
        ...
        distance = np.linalg.norm(v1 - v2) #求两个向量的欧氏距离
    ...
    def predict(self, node_value, node_name='new_node'):
        ...
        for idx in range(0, self.num_classes):
            ...
            variance_list.append(distance)
            self.G.remove_node(node_name)

        result = variance_list.index(min(variance_list))
