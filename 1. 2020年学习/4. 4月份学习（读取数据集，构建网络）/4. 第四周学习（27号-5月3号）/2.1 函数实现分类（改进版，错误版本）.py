...
def node_insert(g, nbrs, k,  X_predict, Y_predict):
    ...
    注意：下面程序中的新添加的节点的nodeindex是从0-29，所以添加节点的时候
    ever_index 只会和0-29的节点添加边（而这里的0-29是原图中的0-29个节点，并非新添加进来的30各节点），所以新添加进来的节点没有任何的连边
    而原来的图中的节点成收缩状态。
    而我们想要的是和新添加进来的节点添加边，所以，
    第一步：是需要将添加进来的节点编号从120开始，121， 122， 、、150.
    第二步：让ever_index和120， 121 、、这些新添加进来的节点进行连边
    改正：将nodeindex变量换成len(list（g.nodes()）)变相的让节点从120开始，同时g.nodes()一直在增加，因为不断地添加新的节点
    g.nodes（）最初是120个，循环一次加1，最后是0-149，总共150个数据
    """
