...
class QuadTreeSolution(object):
    ...
    def __init__(self, image_grid, variance_threshold):

        self.image_grid = image_grid
        self.variance_threshold = variance_threshold  #方差阈值
        self.hisogram = {}  #存储各级别方块个数
    ...
    def node_construct(self, grid, pos, grid_size):
        ...
        if grid_size == 1 or grid_variance <= self.variance_threshold:
            if self.hisogram.get(grid_size) == None: #如果为空，就直接添加值，
                self.hisogram[grid_size] = 1
            else:
                self.hisogram[grid_size] += 1    #如果不为空，直接在原来的块级别上加1，统计这一级别的个数
    ...
    def bfs_for_segmentation(self):
        ...
        plt.imshow(self.image_grid, "gray")
        #cv2.waitKey(0)  # 防止闪图
        plt.show()
image_path = r"C:\Users\Yan\Desktop\COVID-19\NORMAL\NORMAL (4).png"
# image_path = 'img2.png'
img = cv2.imread(image_path, 0)
img = cv2.resize(img, (512, 512), interpolation=cv2.INTER_CUBIC)
s = QuadTreeSolution(img, 60)
print(s.hisogram)
print(sorted(s.hisogram.keys()))
s.bfs_for_segmentation()
