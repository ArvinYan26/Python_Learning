...
class DataClassification(object):
    ...
    def __init__(self, Y_test, X, y, k, num_class):
        self.k = k
        self.x = X
        self.y = y
        # self.g = []
    ...
    def predict(self, X_test, Y_test=[]):
        ...
        s = []
        measures0 = calculate_measure(self.G0, self.x, self.y, [])
        measures1 = calculate_measure(self.G1, self.x, self.y, [])
        measures2 = calculate_measure(self.G2, self.x, self.y, [])
        s.append(measures0)
        s.append(measures1)
        s.append(measures2)
        print(np.array(s))
        ...
        print("need_classification:", len(self.need_classification), self.need_classification)
    ...
    def classification(self, insert_node_id, result):
        ...
        if count0 == len(adj):
            ...
            if count0 > 0 and count0 < len(adj):
                ...
                measures0 = calculate_measure(self.G0, self.x, self.y, insert_node_id)  # new subgraph self.G0 measures
            ...
            if count1 > 0 and count1 < len(adj):
                ...
                GSG = GetSubgraph(self.num_class, self.X_train, self.G)
                self.G0, self.G1, self.G2 = GSG.get_subgraph()
                measures1 = calculate_measure(self.G1, self.x, self.y, insert_node_id)
            ...
            if count2 > 0 and count2 < len(adj):
                ...
                GSG = GetSubgraph(self.num_class, self.X_train, self.G)
                self.G0, self.G1, self.G2 = GSG.get_subgraph()
                measures2 = calculate_measure(self.G2, self.x, self.y, insert_node_id)
...
if __name__ == '__main__':
    ...
    df = pd.read_csv(r"C:\Users\Yan\Desktop\CovidData_n215.csv")
    ...
    DC = DataClassification(Y_test, X, y, k=2, num_class=3)  # 选择最优的K=2传入模型
    DC.fit(X_train, Y_train)  # 训练模型
    re, acc = DC.predict(X_test)
    end_time = time.time()
