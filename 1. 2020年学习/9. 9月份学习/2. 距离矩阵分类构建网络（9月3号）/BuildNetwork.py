...
class BuildNetwork(object):
    ...
    def build_init_network(self, x, y, mean_li, len_li, class_num):
        ...
        self.x = x
        self.y = y
        self.mean_li = mean_li
        self.l = len_li
        self.class_num = class_num

        for index, instance in enumerate(self.x):
            #print(self.Y_train[index])
            self.G.add_node(index, value=instance, TypeNode="train", label=self.y[index])
        #测试添加节点时是否成功if
        #draw_graph(self.G)
        ...
        for i in range(class_num):
            if i == 0:
                base_id = 0
                current_data =self.x[:self.l[i], :]
                #print(current_data, len(current_data))
                self.G = self.build_train_network(current_data, base_id)

            if i == 1:
                base_id = self.l[i-1]
                current_data = self.x[self.l[i-1]:(self.l[i-1]+self.l[i]), :]
                #print(current_data, len(current_data))
                self.G = self.build_train_network(current_data, base_id)

            if i == 2:
                base_id = self.l[i-i] + self.l[i-1]
                current_data = self.x[(self.l[i-i] + self.l[i-1]):, :]
                #print(current_data, len(current_data))
                self.G = self.build_train_network(current_data, base_id)

        return self.G

    def build_train_network(self, data, base_id):

        Matrix = np.round(euclidean_distances(data), decimals=3)  # 将矩阵中的元素保留相应小数点位数
        print("Matrix_len:", len(Matrix))

        for m in range(self.class_num):
            #print(InBegin, InEnd)
           #print(InBegin)
            for i in range(0, len(data)):
                for j in range(0, len(data)):
                    ...
                    #if Matrix[i][j] > np.min(self.mean_li):
                    if Matrix[i][j] > self.mean_li[m]:
                        # print(mean_li[m])
        ...
        for i in range(len(Matrix)):
            for j in range(len(Matrix)):
                if not Matrix[i][j] == 0:
                    self.G.add_edge(i+base_id, j+base_id, weight=Matrix[i][j])
        self.merge_compents()
        return self.G
        #合并单节点和小组件++++++++++++++++++++++++++++++++++++++++++++++++++
        # steps1：对于单节点，计算每一个单节点的最近的一个同类节点，然后连一条边，合并单节点

    def build_test_net(self, G, insert_node_id, mean):
        """

        :param G:
        :param insert_node_id:
        :return:
        """
        ...
        self.merge_compents()
        adj = [n for n in self.G.neighbors(len(data)-1)]

        return self.G

    def merge_compents(self):
        """"
        #合并单节点和小组件++++++++++++++++++++++++++++++++++++++++++++++++++
        # steps1：对于单节点，计算每一个单节点的最近的一个同类节点，然后连一条边，合并单节点
        """
                ...
                if len(adj) == 0:  # 说明是单节点
                    ...
                    for node_id in self.G.nodes():  # 遍历每一个节点，找最近的节点
                        ...
                        if self.G._node[single_node]["label"] == self.G._node[node_id]["label"]:
                            ...
                            v2, v1 = np.array(self.x[int(node_id)]), np.array(self.x[int(single_node)])
                ...
                for x in range(i+1, len(Gcc)): #防止重复循环，所以从未计算过的子图节点开始计算
                    ...
                    if self.G._node[list(Gcc[i])[0]]["label"] == self.G._node[list(Gcc[x])[0]]["label"]:
                        ...
                        for m in Gcc[x]:
                            ...
                            dist.append(node_pair)
                            #print("x:", x)
                            v2, v1 = np.array(self.x[m]), np.array(self.x[n])
                            d = np.linalg.norm(v2 - v1)
                            dist.append(d)
        ...
        if num > self.class_num:
            ...
            #print(num)
            Gcc = sorted(nx.connected_components(self.G), key=len, reverse=True)
            print("variance:", num - self.class_num)
            for m in range(num):
                #print("m:", m)
