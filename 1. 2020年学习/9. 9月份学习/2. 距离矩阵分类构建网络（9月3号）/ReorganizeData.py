...
def reorganize_data(X_train, Y_train):
    ...
    for idx, instance in enumerate(X_train):
        if Y_train[idx] == 0:
            ...
            #new_data.append(current_data0)
            #new_target.append(current_data0_target)
        if Y_train[idx] == 1:
            current_data1.append(instance)
            current_data1_target.append(1)
            #new_data.append(current_data1)
            #new_target.append(current_data1_target)
        if Y_train[idx] == 2:
            current_data2.append(instance)
            current_data2_target.append(2)
            #new_data.append(current_data2)
            #new_target.append(current_data2_target)
