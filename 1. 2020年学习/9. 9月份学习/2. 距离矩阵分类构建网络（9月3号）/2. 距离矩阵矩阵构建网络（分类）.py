...
class Classification(object):
    ...
    def calculate_dis(self, G, insert_node_id):
        ...
        for i in range(len(data)):
            for j in range(i+1, len(data)):
                ...
                t.append(dis)
        mean1 = np.mean(t)   #加入新节点后的整个网络数据之间的平均距离
        print("mean1:", mean1)
        ...
        for i in G.nodes():
            ...
            if not i == insert_node_id:
                ...
                t2.append(dis)
        print("t2:", len(t2))
        mean_val = np.mean(t2)  #新节点与插入子网络中所有节点之间的平均距离
        ...
        for i in G.nodes():
            ...
            #print("count:", count)
            if not i == insert_node_id:
                dist = np.linalg.norm(np.array(self.x[i]) - self.x[insert_node_id])  # x[insert_node_id]:新插入的数据在原始数据集中的位置数据
                dis.append(dist)
                s1.append(i)
                #print(dis, measures[len(measures)-1])
                if dist < mean1: #小于所有数据两两之间的平均距离
                #if dis < measures[len(measures)-1]:
                #if dist < mean_val:
                    self.G.add_edge(i, insert_node_id, weight=dist)
                    s.append(i) #存储新节点邻居的节点编号
        if s == []:

            adj = [n for n in self.G.neighbors(insert_node_id)]
            print("adj:", adj)
            # 说明是单节点，那么久连一条边，将影响变为最大
            if adj == []:
                print("dis:", dis)
                print("np.min:", np.min(dis))
                adj_index = dis.index(np.min(dis))
                print("adj_index:", adj_index)
                adj_id = s1[adj_index]
                print("adj_id:", adj_id)
                s.append(adj_id)
                self.G.add_edge(adj_id, insert_node_id, weight=np.min(dis))
    ...
    def predict(self, X_test, Y_test):
        ...
        #print("graph：", self.G0, self.G1, self.G2)
        ...
        for idx, instance in enumerate(self.X_test):
            ...
            #添加新节点
            # 暂时添加label，等分类结果出来以后在确定label具体是几
            self.G.add_node(insert_node_id, value=instance, typeNode="test", label=0)
            print(len(self.G.nodes))
            dis_cam = []
            #插入到网络1中
            #self.adj0 = self.calculate_dis(self.G0, insert_node_id, self.measures0[len(self.measures0)-1])
            self.adj0 = self.calculate_dis(self.G0, insert_node_id)
            ...
            self.G.add_node(insert_node_id, value=instance, typeNode="test", label=1)
            print("G1:", len(self.G1.nodes))
            #self.mean1 = self.calculate_dis(self.G1, insert_node_id, self.measures1[len(self.measures1)-1])
            self.adj1 = self.calculate_dis(self.G1, insert_node_id)
            #print(self.adj1)
            GS = GetSubgraph(self.G, self.class_num)
            self.G0, self.G1, self.G2 = GS.get_subgraph()  # 得到子图
            ...
            if insert_node_id in self.G.nodes():
                self.G.remove_node(insert_node_id)
            self.G.add_node(insert_node_id, value=instance, typeNode="test", label=2)
            #self.mean2 = self.calculate_dis(self.G2, insert_node_id, self.measures2[len(self.measures2)-1])
            self.mean2 = self.calculate_dis(self.G2, insert_node_id)
            #print(self.adj2)
            GS = GetSubgraph(self.G, self.class_num)
            self.G0, self.G1, self.G2 = GS.get_subgraph()  # 得到子图
