...
class NetworkBaseModel():
    ...
    def predict(self, x: np.ndarray):
        ...
        for item in x:  # 遍历测试数据
            ...
            y_pred.append(int(np.argmin(diff)))   #返回数组中最小值得索引
    ...
    def draw_g(self):
        ...
        G = nx.disjoint_union_all(self.G_list)  #将邻接矩阵形成的图都话在一张图中
        pos = nx.spring_layout(G)

        nx.draw_networkx(G, pos, with_labels=True,
                         font_size=12, node_size=300)  #
...
def irist():
    ...
    print(model.check(x_test,y_test))
