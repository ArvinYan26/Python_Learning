...
class NetworkBaseModel():
    ...
    def fit(self, x: np.ndarray, y:np.ndarray):
        ...
        "1. build network"
        for ith_class in labels:   #ith_class:如果label从1开始，ith_class就是从1开始
    ...
    def predict(self, x: np.ndarray):
        ...
        for item in x:  # 遍历测试数据
            ...
            #print("self.net_measures:", len(self.net_measures), self.net_measures)
            #print("new_measures:", len(new_measures), new_measures)

            diff = paired_euclidean_distances(self.net_measures, new_measures)
            label = int(np.argmin(diff)) + 1   #如果lable从1开始，预测的list的下标就得多加1
...
if __name__ == '__main__':
    ...
    df = pd.read_csv(r"C:\Users\Yan\Desktop\Brazil study files\Dataset\glass.csv")


    features = list(df.columns)
    #zoo数据集第一列是动物种类的名字,所以从第二列开始读取数据
    ...
    for i in range(30):
        ...
        #比例太高，就会出现某个类别训练集中数据只有一个，就无法计算网络的measures，出现Nan,
