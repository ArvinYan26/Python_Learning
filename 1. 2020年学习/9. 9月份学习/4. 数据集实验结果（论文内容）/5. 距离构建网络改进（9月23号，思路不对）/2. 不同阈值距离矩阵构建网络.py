...
class NetworkBaseModel():
    ...
    def fit(self, x: np.ndarray, y:np.ndarray):
        ...
        for ith_class in labels:
            ...
            self.mean_dis_list.append(mean_dis*self.threshold)  #平均差别
    ...
    def predict(self, x: np.ndarray):
        ...
        for item in x:  # 遍历测试数据
            ...
            for ith_class in range(self.num_classes):
                ...
                aver_dis = np.mean(dis_matrix[0])*self.threshold  # 新节点的平均距离
...
if __name__ == '__main__':
    ...
    #print(x.shape, len(y))
    df = pd.read_csv(r"C:\Users\Yan\Desktop\fractal_demension.csv")
    ...
    for threshold in c:
        ...
        print(min, max)
    print("all:", np.round(np.array(b), decimals=4))
    print("ave:", np.round(np.array(d), decimals=4))
    end = time.time()
