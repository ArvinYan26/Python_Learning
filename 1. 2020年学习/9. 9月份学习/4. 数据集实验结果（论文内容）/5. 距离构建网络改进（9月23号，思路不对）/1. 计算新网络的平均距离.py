...
class NetworkBaseModel():
    ...
    def fit(self, x: np.ndarray, y:np.ndarray):
        ...
        for ith_class in labels:
            ...
            "adjacency matrix"
            dataset = x[idxs]
            #dataset = self.data_preprcess(x)[idxs]
            self.data_list.append(dataset)  #存储原始数据
        ...
        self.draw_g()
        print("初始化数据长度：", len(self.data_list[0]))
    ...
    def predict(self, x: np.ndarray):
        ...
        for idx_n, item in enumerate(x):  # 遍历测试数据

            l = self.train_y[idx_n]
            print("label:", l)
            ...
            # 分别插入到三个网络当中
            for ith_class in range(self.num_classes):
                idx = len(self.data_list[ith_class])
                ...
                dis_matrix = euclidean_distances(item, self.data_list[ith_class])  #如果是新节点怎么办？？？？节点编号
                ...
                #将新数据与新网络数据合并计算平均距离
                self.data_list[ith_class] = np.vstack((self.data_list[ith_class], item))
                #print("添加新节点后：", len(self.data_list[0]))
                adj_matrix = euclidean_distances(self.data_list[ith_class], self.data_list[ith_class])
                # print("adj_matrix:", adj_matrix)
                # 要先求两两平均距离，后面会改动数据。
                mean_dis = np.sum(adj_matrix) / (len(adj_matrix) ** 2 - len(adj_matrix))*self.t

                #aver_dis = np.mean(dis_matrix[0])  # 新节点与初始网络中每个数据间的距离的平均距离
                #aver_dis = aver_dis*self.t
                #print("aver_dis:", aver_dis)
                min_idx = int(np.argmin(dis_matrix[0]))         # 最小距离节点编号

                #edge_idxs = list(np.argwhere(dis_matrix[0] < self.mean_dis_list[ith_class]))
                #edge_idxs = list(np.argwhere(dis_matrix[0] < aver_dis))

                edge_idxs = list(np.argwhere(dis_matrix[0] < mean_dis))  #最小距离节点编号
                ...
                #将新数据删除
                self.data_list[ith_class] = np.delete(self.data_list[ith_class], len(self.data_list[ith_class])-1, axis=0)
                print("删除新节点后：", len(self.data_list[0]))
                print("="*10)
            ...
            #确定分类后，计算新网络的measures，添加新数据到相应类别中，添加新节点到新的相应数据中
            #是新节点的节点编号等于每一类网络的数据长度

            #self.classification(label, neighbor, item, new_measures)
            #self.draw_g()
        ...
        print("新数据长度：", len(self.data_list[0]))
        #print("count:", count)
    ...
    def classification(self, label, neighbor, item, new_measures):
        #print("label:", label)
        #for ith_class in range(self.num_classes):
        ...
        for ith_class in range(self.num_classes):
            if label == ith_class:
                idx = len(self.data_list[ith_class])
                # 分类结束后，确定是某个类别后，将新数据添加到子网络数据集中
                self.data_list[ith_class] = np.vstack((self.data_list[ith_class], item))
                #self.net_measures[ith_class].append(new_measures[ith_class]) #将所有新的measures存储器起来
                self.net_measures[ith_class] = new_measures[ith_class]
...
if __name__ == '__main__':
    ...
    #print(x.shape, len(y))
    #df = pd.read_csv(r"C:\Users\Yan\Desktop\CovidData_n115.csv")
    #df = pd.read_csv(r"C:\Users\Yan\Desktop\fractal_demension_145_175_5.csv")
