...
class NetworkBaseModel():
    ...
    def fit(self, x: np.ndarray, y:np.ndarray):
        ...
        for ith_class in labels:
            ...
            for i in range(len(dataset)):
                for j in range(i + 1, len(dataset)):
                    if not i == j:
                        dis = np.linalg.norm(dataset[i] - dataset[j])
                        s.append(dis)
            mean_value = np.mean(s)  # 计算方差
            mean = round(mean_value, 3)
            """
            mean_demension = [np.mean(dataset)]   #求出每一类的平均demension
            self.ave_d.append(mean_demension)
        ...
        print("self.ave_d:", np.array(self.ave_d))
        self.draw_g()
    ...
    def predict(self, x: np.ndarray):
        ...
        for idx, item in enumerate(x):  # 遍历测试数据
            ...
            #print("item:", item)
            #np.mean(item) < np.median(self.ave_d):
            #step1: 先分covid-19数据
            #print(np.array([[np.mean(item)]]).shape, np.array(self.ave_d).shape)
            #print(np.array([np.mean(item)]), np.array(self.ave_d))

            item_mean_d = np.array(np.mean(item)).reshape(1, -1)
            mean_d = euclidean_distances(item_mean_d, np.array(self.ave_d))
            if int(np.argmin(mean_d[0])) == 2:
                y_pred.append(2)
                print("covid:", 2)
                print("="*30)
                ...
                # 分别插入到正常和普通病毒感染的类别里面进行分类
                for ith_class in range(self.num_classes - 1):
                    ...
                    # 添加节点， 添加连边
                    test_node = ("new_node", {'value': None, 'class': 'test', 'type': 'test'})

                    self.G_list[ith_class].add_nodes_from([test_node])

                    edges = [("new_node", min_idx)]
                    for edge_idx in edge_idxs:
                        edges.append(("new_node", int(edge_idx)))
                    ...
                    #self.draw_g()
                    self.G_list[ith_class].remove_node("new_node")
                ...
                diff = paired_euclidean_distances(np.delete(self.net_measures, 2, axis=0), new_measures)
                #print("diff:", diff)
...
if __name__ == '__main__':
    ...
    #print(x.shape, len(y))
    df = pd.read_csv(r"C:\Users\Yan\Desktop\fractal_demension_100_170_7.csv")
    #df = pd.read_csv(r"C:\Users\Yan\Desktop\fractal_demension_110_200_5.csv")
