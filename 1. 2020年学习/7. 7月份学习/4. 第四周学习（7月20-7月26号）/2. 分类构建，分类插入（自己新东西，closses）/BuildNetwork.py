...
def node_insert(G, k, nbrs, instance, base_index, insert_node_id, label):
    """
    ...
    """
    # 插入新的节点构建连边
    G.add_node(insert_node_id, values=instance, NodeType='test', label=label)
    ...
    print("radius_indices:", radius_indices)
    ...
    if (len(radius_indices)) > k:  # 判断用哪种方法连边构图,因为,radius应用于稠密区域，邻居大于k的话就是稠密
        ...
        for indiceNode, indicesNode in enumerate(radius_indices):
            for tmpi, indice in enumerate(indicesNode):
                if indice == indiceNode:
                    continue
                G.add_edge(indice + base_index, insert_node_id, weight=radius_distances[indiceNode][tmpi])
    else:
        for indiceNode, indicesNode in enumerate(knn_indices):
            for tmpi, indice in enumerate(indicesNode):
                if (str(indice) == str(indiceNode)):
                    continue
                G.add_edge(indice + base_index, insert_node_id, weight=knn_distances[indiceNode][tmpi])
    return G
