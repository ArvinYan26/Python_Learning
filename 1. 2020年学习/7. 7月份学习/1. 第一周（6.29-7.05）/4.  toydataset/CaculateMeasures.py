...
def calculate_measure(G, insert_node_id):
    """
    :param net: 构建的网络g
    ...
    #紧密中心性
    closeness = nx.closeness_centrality(G)  #越大越重要
    print(closeness)
    #print(insert_node_id)
    a = closeness[str(insert_node_id)]
    measures.append(a)

    eigenvector = nx.eigenvector_centrality(G)    #越大越好
    b = eigenvector[str(insert_node_id)]
    #measures.append(b)

    betweenness = nx.betweenness_centrality(G)  #越大越重要
    c = betweenness[str(insert_node_id)]
    #measures.append(c)
