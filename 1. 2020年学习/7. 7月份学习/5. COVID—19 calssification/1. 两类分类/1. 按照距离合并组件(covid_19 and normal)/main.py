...
class DataClassification(object):
    ...
    def predict(self, X_test, Y_test=[]):
        ...
        draw_graph(self.G)
        #steps2：获得子图
        self.G0, self.G1 = GSG.get_subgraph()
        ...
        #得到最后的图和子图
        draw_graph(self.G)
        print("nodes_num:", len(self.G.nodes()))
        GSG = GetSubgraph(self.num_class, self.X_train, self.Y_train, self.G, self.k, self.nbrs)
        self.G0, self.G1 = GSG.get_subgraph()
        """
        draw_graph(self.G0)
        ...
        """
        #得到准确率，和预测target
        print(len(self.Y_test), len(result))
        ...
        if count0 == len(adj):
            ...
            if count0 > 0 and count0 < len(adj):
                ...
                for n in neighbor:
                    self.G.add_edge(str(insert_node_id), n)
                GSG = GetSubgraph(self.num_class, self.X_train, self.Y_train, self.G, self.k, self.nbrs)
                self.G0, self.G1 = GSG.get_subgraph() #get the new sungraph to calclulate the measures
...
if __name__ == '__main__':
    ...
    DC = DataClassification(Y_test, 4, num_class=2)  # 选择最优的K=2传入模型
