...
def split_data(data, data_target, data_len, per_class_len, X_train, Y_train):
    ...
    for j in range(class_num):
        ...
        for i in li0:
            ...
            data_target = np.delete(data_target, data_target[i], axis=0)
    ...
    return X_train, Y_train, data, data_target
X_train1, Y_train1, data, data_target = split_data(iris_data, iris_target, data_len, per_class_len, X_train1, Y_train1)
print(len(data), len(data_target))
print("训练据集：")
print(np.array(X_train1), np.array(Y_train1))


X_train2, Y_train2, data, data_target= split_data(X_train1, Y_train1, len(X_train1), len(X_train1)/3, X_train2, Y_train2)
print("训练集")
