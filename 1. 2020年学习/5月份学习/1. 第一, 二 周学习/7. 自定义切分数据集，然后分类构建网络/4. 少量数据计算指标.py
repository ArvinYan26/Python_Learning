...
class DataClassification(object):
    ...
    def __init__(self, k, calss_num):
        self.k = k
        self.X_net, self.Y_net, self.X_items, self.Y_items = self.get_iris()
        self.data_len = len(self.X_net)  # 此程序是24
        self.num_class = calss_num
        self.per_class_data_len = self.data_len / self.num_class
        self.nbrs = []
        self.radius = []
        self.nodes_list = []
        self.edges_list = []
    ...
    def build_X_train_network(self):
        ...
        for i in range(self.num_class): #按类别循环遍历每一类别的每一个数据
            ...
            self.nbrs.append(temp_nbrs)
            distances, indices = self.KNN(temp_nbrs, current_data)
            #print(distances, indices)
            temp_radius = self.get_radius(distances)
            self.radius.append(temp_radius)
            ...
            #添加连边
            for index, one_data in enumerate(current_data):
                if len(radius_indices[index]) > (self.k-1):  #判断用哪种方法连边构图,因为,radius应用于稠密区域，邻居大于k的话就是稠密
                    for index, nbrs_indices in enumerate(radius_indices):
                        for indices, eve_index in enumerate(nbrs_indices):
                            if index == eve_index:  #如果是本身，就跳过，重新下一个循环
                                continue
                            edge = (index + base_index, eve_index + base_index,radius_distances[index][indices] )
                    ...
                    for index, nbrs_indices in enumerate(indices):
                        for indices, eve_index in enumerate(nbrs_indices):
                            if index == eve_index: #如果是本身，就跳过，重新下一个循环
                                continue
                            edge = (index + base_index, eve_index + base_index, distances[index][indices])
                            self.edges_list.append(edge)

        self.g.add_weighted_edges_from(self.edges_list)
        #self.generat_net_measures()
        print(self.g.nodes())
        color_list = [self.color_map[self.g.nodes[node]['class_num']] for node in self.g.nodes()]
        #plt.subplot(211)
        nx.draw_networkx(self.g, pos=self.X_net, node_color=color_list, with_labels=False)
        plt.title("X_net")
        plt.show()
        #print(self.g.nodes())
