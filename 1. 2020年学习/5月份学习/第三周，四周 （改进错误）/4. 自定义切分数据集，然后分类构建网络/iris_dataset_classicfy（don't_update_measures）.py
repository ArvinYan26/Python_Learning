...
class DataClassification(object):
    ...
    def plot_train_network(self, G, class_num):

        nx.draw_networkx(G, node_color=self.color_map[class_num], with_labels=True, node_size=300) #节点默认大小为300
        plt.title("X_net")
        plt.show()
        print(G.nodes())
        print("X_net节点数：", len(G.nodes()))
    ...
    def classicfication(self, instance, node_name="new_node"):
        ...
        class_num = int(distances_list.index(min(distances_list)))
        print("类别名：", class_num)
        #print("result:", result)
        if class_num == 0:
    ...
    def accuracy(self):
        ...
        for i in range(len(self.Y_items)):
            if self.predict_target[i] == self.Y_items[i]:
                count += 1
        print(count)
        accuracy = round(count / len(self.Y_items), 3) #保留是哪位小数
