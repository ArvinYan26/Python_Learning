...
class DataClassification(object):
    ...
    def __init__(self, k, num_class):
        ...
        self.weight_alpha = [0.3, 0.4, 0.3]  # mesures weight
    ...
    def get_iris(self):
        ...
        train_data, train_target, X_train1, Y_train1 = split_data(iris_data, iris_target, X_train1, Y_train1,
                                                                  train_data, train_target)
        # print("训练集：")
        # print(np.array(X_train1), np.array(Y_train1))

        print("train_dataset:")
        print(len(np.array(train_data)))
        print(np.array(train_data), np.array(train_target))
    ...
    def build_X_items_network(self, G, class_num, instance):
        ...
        print("new_node_values：", instance)
        ...
        """
        # add weight
        # v1, v2 = np.array(net_measure)*self.weight_alpha, np.array(self.net_measure[class_num])*self.weight_alpha

        # no weight
        v1, v2 = np.array(net_measure), np.array(self.net_measure[class_num])

        # euclidean_distances = np.linalg.norm(v1 - v2)  # 插入后减去插入前
        euclidean_distances = sum([abs((v2[i]-v1[i])/v1[i]) for i in range(len(v1))])
    ...
    def classicfication(self, instance):
        ...
        for per_class in range(self.num_class):
            ...
            # 此处是将节点插入到每一类网络中去
            if per_class == 0:
                euclidean_distances, v1, v2 = self.build_X_items_network(self.G0, per_class, instance)
                print("add_new_node：", len(self.G0.nodes()))
                print(self.G0.edges())
                print("add_new_node_mesures:", v1)
                print("original_mesures:", v2)
                print("mesures_distance:", euclidean_distances)

                distances_list.append(euclidean_distances)
                self.G0.remove_node(self.node_insert_num)
                print("remove_new_node：", len(self.G0.nodes()))
                print(" ")

            if per_class == 1:
                euclidean_distances, v1, v2 = self.build_X_items_network(self.G1, per_class, instance)
                print(len(self.G1.nodes()))
                print(self.G1.edges())
                print("add_new_node_mesures:", v1)
                print("original_mesures:", v2)
                ...
                print("remove_new_node:", len(self.G1.nodes()))
                print(" ")
            ...
            if per_class == 2:
                ...
                print("remove_new_node:", len(self.G2.nodes()))
                print(" ")

        print("measures_distance：", distances_list)
        class_num = distances_list.index(min(distances_list))
        print("classify_result：", class_num)

        if class_num == 0:
            print("before_classify：", len(self.G0.nodes()))
            self.build_X_items_network(self.G0, class_num, instance)
            print("after_classify：", len(self.G0.nodes()))
            self.predict_target.append(class_num)
            # 插入节点后，网络measures变化，所以需要更新原来字典里的measures
            # when I add new node to this class, I need to update this network's measures.
            self.net_measure[class_num] = self.calculate_measure(self.G0)
        ...
        if class_num == 1:
            print("before_classify：", len(self.G1.nodes()))
            self.build_X_items_network(self.G1, class_num, instance)
            print("after_classify：", len(self.G1.nodes()))
            self.predict_target.append(class_num)
            # 插入节点后，网络measures变化，所以需要更新原来字典里的measures
            # when I add new node to this class, I need to update this network's measures.
            self.net_measure[class_num] = self.calculate_measure(self.G1)
            print(self.net_measure)
        ...
        if class_num == 2:
            print("before_classify：", len(self.G2.nodes()))
            self.build_X_items_network(self.G2, class_num, instance)
            print("after_classify:", len(self.G2.nodes()))
            self.predict_target.append(class_num)
            # 插入节点后，网络measures变化，所以需要更新原来字典里的measures
            # when I add new node to this class, I need to update this network's measures.
    ...
    def accuracy(self):
        """calculate accuracy """
        print("original_label:", self.Y_items)
