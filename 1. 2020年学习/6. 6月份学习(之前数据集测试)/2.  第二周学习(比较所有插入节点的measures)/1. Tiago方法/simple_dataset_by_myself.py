...
class DataClassification(object):
    ...
    def __init__(self, k, num_class):
        ...
        self.net0_measure = []   #存储每一类别插入Y_items中的数据时的影响值
        self.similarity0 = []
        self.net1_measure = []
        self.similarity1 = []
        self.G0 = nx.Graph()
    ...
    def add_X_items_data(self):
        ...
        for index, instance in enumerate(self.X_items):
            ...
            if self.Y_items[index] == 0:
                ...
                measure0.append(net0_measure)
                #self.G0.remove_node(self.node_insert_num)
                #print(np.array(measure0))
                #net0_measure = max(measure0)
        ...
        for i in range(len(self.net0_measure)-1):
            ...
            distance0 = np.linalg.norm(M1-M0)
            self.similarity0.append(distance0)
            ...
            distance1 = np.linalg.norm(N1 - N0)
            self.similarity1.append(distance1)

            #variation1 = (N1 - N0) / N0
            #self.impact1.append(variation1)

        print("self.similarity0:")
        print(np.array(self.similarity0))
        print("self.similarity1:")
        print(np.array(self.similarity1))
        print("="*100)
    ...
    def classicfication(self, instance):
        ...
        net0_measure = self.calculate_measure(self.G0)
        self.net0_measure.append(net0_measure)
        print("self.net0_measure:", np.array(self.net0_measure))
        print("add_new_bode:")
        ...
        print("similarity0 length:", self.similarity0)
        ...
        """
        #计算欧氏距离平均值
        mean_0 = np.mean(self.similarity0)
        print("mean_0:", mean_0)
        distances_list.append(mean_0)

        self.net0_measure.remove(self.net0_measure[len(self.net0_measure)-1])
        print("self.net0_measure:", np.array(self.net0_measure))
        self.similarity0.remove(self.similarity0[len(self.similarity0)-1])
       #distances_list.append(euclidean_distances)
        ...
        net1_measure = self.calculate_measure(self.G1)
        self.net1_measure.append(net1_measure)
        print("self.net1_measure:", self.net1_measure)
        print("add_new_bode:")
        print(self.net1_measure)
        ...
        # 计算插入节点后的measures和之前的measures的variation0(差异：variation)
        i = len(self.net1_measure)
        print(self.net1_measure[i - 1], self.net1_measure[i - 2])
        N1 = np.array(self.net1_measure[i - 1])
        N0 = np.array(self.net1_measure[i - 2])
        distance1 = np.linalg.norm(N1 - N0)
        self.similarity1.append(distance1)
        """
        euclidean_distance = []
        ...
        print(euclidean_distance)
        """
        mean_1 = np.mean(self.similarity1)
        print("mean_1:", mean_1)
        distances_list.append(mean_1)
        print("长度:",len(self.similarity0))
        self.net1_measure.remove(self.net1_measure[len(self.net1_measure) - 1])
        print("self.net1_measure:", self.net1_measure)
        self.similarity1.remove(self.similarity1[len(self.similarity1) - 1])
        ...
        if class_num == 0:
            ...
            print(np.array(self.net0_measure))
            self.similarity0.append(distance0)
            print("self.similarity0:", np.array(self.similarity0))
        ...
        if class_num == 1:
            ...
            print(np.array(self.net1_measure))
            self.similarity1.append(distance1)
            print("self.similarity1:", np.array(self.similarity1))
