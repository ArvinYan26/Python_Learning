...
class DataClassification(object):
    ...
    def draw_graph(self, G):
        ...
        nx.draw_networkx(G, pos, node_color=color_list, with_labels=True,
                         node_size=80, font_size=6)  # 节点默认大小为300,节点标签默认大小12
    ...
    def get_subgraph(self):
        ...
        """
        if num > self.num_class:  # 如果组件数大于类别数执行下面步骤,正常情况下，分类阶段不会用到，因为很明显的分3类
            for m in range(num):
    ...
    def classification(self, insert_node_id, label):
        ...
        #先检查单节点
        #if len(adj) == 0:  #插入的节点是单节点（可以酌情考虑，因为正常情况下不该出现这种情况）
        count0 = 0
        count1 = 0
        count2 = 0
        for n in adj:
...
if __name__ == '__main__':
    DC = DataClassification(1, 3)
    DC.accuracy()
    #list0 = []
    """
    for i in range(5):
        accurancy = DC.accuracy()
        list0.append(accurancy)
    print("accurancy list:", list)
    """