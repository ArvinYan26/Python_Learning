...
class DataClassification(object):
    ...
    def classification(self, insert_node_id, label):
        new_id = str(insert_node_id)  # 因为在构建离岸边的时候用的就是str()形式，所以需要此处需要转化为字符串
        print(self.G.nodes())
        adj = [n for n in self.G.neighbors(new_id)]  # find the neighbors of the new node
        print("adj:", adj)
        ...
        if count0 == len(adj):
           ...
           # if count0 == 0:
            ...
            if count0 >= 0 and count0 < len(adj):
                self.G.remove_node(new_id) #delate the edges and node
                ...
                for n in neighbor:
                    self.G.add_edge(new_id, n)  #add edges
                self.get_subgraph()

                measures0 = self.calculate_measure(self.G0)  #new measures
