...
class DataClassification(object):
    ...
    def get_data(self):
        ...
        train_data, X_test, train_target, Y_test = train_test_split(iris_data, iris_target, test_size=0.3)
    ...
    def build_init_network(self, label):
        ...
        for idx, one_data in enumerate(self.train_data):  # 这个语句仅仅是获取索引indx，然后给他连边
            ...
            if (len(radius_indices[idx])) > self.k:  # 判断用哪种方法连边构图,因为,radius应用于稠密区域，邻居大于k的话就是稠
    ...
    def draw_graph(self, G):
        ...
        plt.savefig('classification result')
        plt.clf()
        plt.show() #重置画布，
