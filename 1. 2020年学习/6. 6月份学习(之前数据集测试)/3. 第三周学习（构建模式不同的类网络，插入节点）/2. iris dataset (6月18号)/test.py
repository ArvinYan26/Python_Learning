...
class DataClassification(object):
                ...
                for indiceNode, indicesNode in enumerate(radius_indices):
                    for tmpi, indice in enumerate(indicesNode):
                            if (str(indice) == str(indiceNode)):
                                continue
                            if (self.G.nodes()[str(indice)]["label"] == self.G.nodes()[str(indiceNode)]["label"] or not label):
    ...
    def draw_graph(self, G):
        ...
        nx.draw_networkx(G, pos, node_color=color_list, with_labels=True,
                         font_size=8, node_size=300)  # 节点默认大小为300, font_size默认是12
        plt.show()
    ...
    def classification(self, insert_node_id, new_node_label):
        ...
        for n in adj:
            if n in self.G0.nodes():
                ...
                count2 += 1
        if count0 == len(adj) or count1 == len(adj) or count2 == len(adj):
            print("check_label:", new_node_label)
            ...
            print("self.G0_info:")
            self.draw_graph(self.G0)
            print("self.G0.nodes:", self.G0.nodes)

            print("self.G1_info:")
            self.draw_graph(self.G1)
            print("self.G1.nodes:", self.G1.nodes)

            print("self.G2_info:")
            self.draw_graph(self.G2)
            print("self.G2.nodes:", self.G2.nodes)
