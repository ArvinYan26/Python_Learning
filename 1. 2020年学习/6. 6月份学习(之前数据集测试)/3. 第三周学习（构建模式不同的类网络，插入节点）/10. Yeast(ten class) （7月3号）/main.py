...
class DataClassification(object):
    ...
    def __init__(self, k, num_class):
        ...
        self.color_map = {0: 'red', 1: 'green', 2: 'yellow', 3: 'b', 4: 'm', 5: 'c', 6: 'black',
        ...
        for n in range(num):
            ...
            for m in G.nodes():
                ...
                if G._node[m]["label"] == 7:
                    ...
                    if count7 >= 2:
                        self.G7 = G
                if G._node[m]["label"] == 8:
                    count8 += 1
                    if count8 >= 2:
                        self.G8 = G
                if G._node[m]["label"] == 9:
                    count9 += 1
                    if count9 >= 2:
                        self.G9 = G
                ...
                for n in G.nodes():
                    ...
                    if G._node[n]["label"] == 7:
                        ...
                        for a in self.G7.nodes():
                            ...
                            if count == self.k:
                                break
                    if G._node[n]["label"] == 8:
                        count = 0
                        for a in self.G8.nodes():
                            if n == a:
                                continue
                            self.G.add_edge(str(n), str(a))
                            count += 1
                            if count == self.k:
                                break
                    if G._node[n]["label"] == 9:
                        count = 0
                        for a in self.G9.nodes():
                            if n == a:
                                continue
                            self.G.add_edge(str(n), str(a))
                            count += 1
    ...
    def predict(self, X_test, Y_test):
        ...
        for index, instance in enumerate(self.X_test):
            ...
            measures6 = calculate_measure(self.G6)
            self.net6_measure.append(measures6)
            measures7 = calculate_measure(self.G7)
            self.net7_measure.append(measures7)
            measures8 = calculate_measure(self.G8)
            self.net8_measure.append(measures8)
            measures9 = calculate_measure(self.G9)
            self.net9_measure.append(measures9)
            # 插入新的节点构建连边
            insert_node_id = len(list(self.G.nodes()))
    ...
    def classification(self, insert_node_id, label):
        ...
        for n in adj:
            if n in self.G0.nodes():
                ...
                print("label:", label)
                count6 += 1
            elif n in self.G7.nodes():
                label = self.G._node[n]["label"]
                print("label:", label)
                count7 += 1
            elif n in self.G8.nodes():
                label = self.G._node[n]["label"]
                print("label:", label)
                count8 += 1
            elif n in self.G9.nodes():
                label = self.G._node[n]["label"]
                print("label:", label)
                count9 += 1
        ...
        if count0 == len(adj):
            ...
            for n in adj:
                self.G.add_node(str(insert_node_id), typeNode='test', label=label)
                self.G.add_edge(str(insert_node_id), n)
            self.predict_label.append(8)
        elif count9 == len(adj):
            print("classification_result:", 9)
            self.G.remove_node(str(insert_node_id))
            for n in adj:
                self.G.add_node(str(insert_node_id), typeNode='test', label=label)
                self.G.add_edge(str(insert_node_id), n)
            self.predict_label.append(9)
        else:
            print("模糊分类情况：")
            draw_graph(self.G, self.color_map)
            ...
            if count7 >= 0 and count7 < len(adj):
                if str(insert_node_id) in self.G.nodes():
                    self.G.remove_node(str(insert_node_id))
                node_list = self.G7.nodes()
                neighbor = [x for x in node_list if x in adj]
                # 添加到2类网络中
                ...
                measures7 = calculate_measure(self.G7)
                M1, M7 = np.array(self.net7_measure[len(self.net7_measure) - 1]), np.array(measures7)
                print("M1, M7:", M1, M7)
                euclidean_dist7 = np.linalg.norm(M7 - M1)
                dist_list.append(euclidean_dist7)

            if count8 >= 0 and count8 < len(adj):
                if str(insert_node_id) in self.G.nodes():
                    self.G.remove_node(str(insert_node_id))
                node_list = self.G8.nodes()
                neighbor = [x for x in node_list if x in adj]
                # 添加到2类网络中
                ...
                measures8 = calculate_measure(self.G8)
                M1, M8 = np.array(self.net8_measure[len(self.net8_measure) - 1]), np.array(measures8)
                print("M1, M8:", M1, M8)
                euclidean_dist8 = np.linalg.norm(M8 - M1)
                dist_list.append(euclidean_dist8)

            if count9 >= 0 and count9 < len(adj):
                if str(insert_node_id) in self.G.nodes():
                    self.G.remove_node(str(insert_node_id))
                node_list = self.G9.nodes()
                neighbor = [x for x in node_list if x in adj]
                # 添加到2类网络中
                ...
                measures9 = calculate_measure(self.G9)
                M1, M9 = np.array(self.net9_measure[len(self.net9_measure) - 1]), np.array(measures9)
                print("M1, M9:", M1, M9)
                euclidean_dist9 = np.linalg.norm(M9 - M1)
                dist_list.append(euclidean_dist9)
...
if __name__ == '__main__':
    ...
    info = []
    fina_result = []
    DC = DataClassification(k=5, num_class=10)
