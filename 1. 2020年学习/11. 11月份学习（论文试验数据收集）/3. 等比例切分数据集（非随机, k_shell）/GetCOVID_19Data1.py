...
def get_data(percent):
    ...
    x_train = np.vstack((data0[:p], data1[:p], data2[:p]))
    #训练集target
    y_train = np.array(data_target0[:p] + data_target1[:p] + data_target2[:p])

    #测试集数据
    x_test = np.vstack((data0[p:], data1[p:], data2[p:]))
    y_test = np.array(data_target0[p:] + data_target1[p:] + data_target2[p:])
    """
