...
class NetworkBaseModel():
    ...
    def fit(self, x: np.ndarray, y:np.ndarray):
        ...
        #print(self.G.edges())d
        #print(len(self.G.nodes()), self.G.nodes())
        self.draw_g()
    ...
    def draw_g(self):
        color_map = {0: 'r', 1: 'g', 2: 'b', 3: 'g', 4: 'm', 5: 'c', 6: 'black',
...
if __name__ == '__main__':
    ...
    model.get_params(1.01, 0.88)
    rho = model.fit(x_train, y_train)
    """
    t = np.arange(0.80, 1, 0.01)
    print("t:", t)
    Rho = []
    for i in t:
        model.get_params(0.92, 0.88)
        rho = model.fit(x_train, y_train)

        Rho.append(rho)
    #x_axis = [i for i in range(len(t))]
    plt.figure()
